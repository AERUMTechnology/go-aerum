...
import (
	...
	"github.com/AERUMTechnology/go-aerum-new/light"
...
func (pm *ProtocolManager) Stop() {
	...
	log.Info("Stopping light AERUMTechnology protocol")
	...
	log.Info("Light AERUMTechnology protocol stopped")
}
...
func (pm *ProtocolManager) handle(p *peer) error {
	...
	p.Log().Debug("Light AERUMTechnology peer connected", "name", p.Name())
	...
	if err := p.Handshake(td, hash, number, genesis.Hash(), pm.server); err != nil {
		p.Log().Debug("Light AERUMTechnology handshake failed", "err", err)
		return err
	}
	...
	if err := pm.peers.Register(p); err != nil {
		p.Log().Error("Light AERUMTechnology peer registration failed", "err", err)
		return err
	}
	defer func() {
	...
	for {
		if err := pm.handleMsg(p); err != nil {
			p.Log().Debug("Light AERUMTechnology message handling failed", "err", err)
			return err
		}
	}
}
...
func (pm *ProtocolManager) handleMsg(p *peer) error {
	...
	p.Log().Trace("Light AERUMTechnology message arrived", "code", msg.Code, "bytes", msg.Size)
