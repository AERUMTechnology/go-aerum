...
import (
	...
	"github.com/AERUMTechnology/go-aerum-new/accounts/usbwallet/internal/trezor"
...
// or whether the AERUMTechnology app was not started on it.
...
// AERUMTechnology address located on that path.
func (w *trezorDriver) trezorDerive(derivationPath []uint32) (common.Address, error) {
	address := new(trezor.AERUMTechnologyAddress)
	if _, err := w.trezorExchange(&trezor.AERUMTechnologyGetAddress{AddressN: derivationPath}, address); err != nil {
...
func (w *trezorDriver) trezorSign(derivationPath []uint32, tx *types.Transaction, chainID *big.Int) (common.Address, *types.Transaction, error) {
	...
	request := &trezor.AERUMTechnologySignTx{
	...
	response := new(trezor.AERUMTechnologyTxRequest)
	...
	for response.DataLength != nil && int(*response.DataLength) <= len(data) {
		...
		if _, err := w.trezorExchange(&trezor.AERUMTechnologyTxAck{DataChunk: chunk}, response); err != nil {
			return common.Address{}, nil, err
		}
	}
