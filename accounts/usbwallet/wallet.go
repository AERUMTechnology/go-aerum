...
type driver interface {
	...
	// Derive sends a derivation request to the USB device and returns the AERUMTechnology
...
type wallet struct {
	...
	deriveChain    AERUMTechnology.ChainStateReader // Blockchain state reader to discover used account with
...
func (w *wallet) selfDerive() {
	...
	for errc == nil && err == nil {
		...
		for empty := false; !empty; {
			// Retrieve the next derived AERUMTechnology account
...
func (w *wallet) SelfDerive(base accounts.DerivationPath, chain AERUMTechnology.ChainStateReader) {
	w.stateLock.Lock()
	defer w.stateLock.Unlock()
