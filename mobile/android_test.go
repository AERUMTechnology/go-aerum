...
public class AndroidTest extends InstrumentationTestCase {
	...
	public void testInprocNode() {
		...
		try {
			...
			Node node = new Node(getInstrumentation().getContext().getFilesDir() + "/.AERUMTechnology", new NodeConfig());
			node.start();
			...
			AERUMTechnologyClient ec = node.getAERUMTechnologyClient();
	...
	// signatures too. Regression test for go-aerum-new issue #14599.
...
func TestAndroid(t *testing.T) {
	...
	for _, dir := range []string{"src/main", "src/androidTest/java/org/AERUMTechnology/gethtest", "libs"} {
	...
	gobind := exec.Command("gomobile", "bind", "-javapkg", "org.AERUMTechnology", "github.com/AERUMTechnology/go-aerum-new/mobile")
	...
	if err = ioutil.WriteFile(filepath.Join("src", "androidTest", "java", "org", "AERUMTechnology", "gethtest", "AndroidTest.java"), []byte(androidTestClass), os.ModePerm); err != nil {
...
<manifest xmlns:android="http://schemas.android.com/apk/res/android"
          package="org.AERUMTechnology.gethtest"
