...
// Package filters implements an AERUMTechnology filtering system for block,
...
type subscription struct {
	...
	typ       Type
	created   time.Time
	logsCrit  AERUMTechnology.FilterQuery
	logs      chan []*types.Log
...
func (es *EventSystem) SubscribeLogs(crit AERUMTechnology.FilterQuery, logs chan []*types.Log) (*Subscription, error) {
...
func (es *EventSystem) subscribeMinedPendingLogs(crit AERUMTechnology.FilterQuery, logs chan []*types.Log) *Subscription {
	sub := &subscription{
		id:        rpc.NewID(),
...
func (es *EventSystem) subscribeLogs(crit AERUMTechnology.FilterQuery, logs chan []*types.Log) *Subscription {
	sub := &subscription{
		id:        rpc.NewID(),
...
func (es *EventSystem) subscribePendingLogs(crit AERUMTechnology.FilterQuery, logs chan []*types.Log) *Subscription {
	sub := &subscription{
		id:        rpc.NewID(),
